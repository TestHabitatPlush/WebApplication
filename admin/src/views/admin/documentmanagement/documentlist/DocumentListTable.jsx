...
//     const docData = res?.data?.data; // Safely extract the first (and expected only) document
  ...
  const onViewHandler = async (documentId) => {
    try {
      ...
      if (docData) {
        ...
        const fullDocUrl = `${baseUrl}/${docData.document}`;
        setShowViewFormData({ ...docData, documentUrl: fullDocUrl });
        setViewModal(true);
      } else {
        console.error("Document not found");
      }
