...
const BulkUserCreation = () => {
  ...
  // 🔹 State for pagination + unit list
  ...
  const handleManualSubmit = async (rows) => {
    try {
      ...
      const users = selectedRows.map((r) => ({
        ...
        lastName: r.lname,
        email: r.email,
        mobileNumber: r.mnumber, // ✅ backend expects mobileNumber
        roleId: r.roleId,
        unitId: r.unitId,
        ...
        societyId: societyId, // optional (backend already gets from params)
      }));

      console.log("🚀 Final Users Sent:", users);

      const res = await createMultipleSocietyUserHandler(societyId, users);

      console.log("Manual users result:", res);
      alert(res?.message || "Users created successfully!");
    } catch (err) {
      console.error("❌ Error creating users manually:", err);
  ...
  const formColumns = [
    ...
    },
    {
      ...
      inputType: "select",
      required: true,
      options: units, // 🔥 Now dynamic from API
    },
    // 🔹 Address fields
        ...
        <div className="w-full">
          ...
          <div className="flex flex-col mt-[35px] space-y-3">
            <div className="flex justify-end gap-8 mr-12">
              <button className="flex gap-2 p-2 text-white bg-green-600 rounded-md">
              ...
              >
                Download Format
                <TiArrowDownOutline className="mt-1" />
              </button>
            </div>
