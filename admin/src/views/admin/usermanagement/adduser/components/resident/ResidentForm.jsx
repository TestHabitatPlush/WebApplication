...
import { getAddressByIdHandler } from "../../../../../../handlers/AddressHandler";
import UserUnitHandler from "../../../../../../handlers/UserUnitHandler";

const ResidentForm = ({ mode = "add", editData = null, onSuccess }) => {
 ...
 const societyId = useSelector((state) => state.auth.user?.societyId);
  ...
  const { createSocietyResidentUserHandler ,updateResidentHandler } = UserHandler();
  const { getUserRolesHandler } = UserRoleHandler();
  const { deleteUnitHandler ,getAllUnitHandler} = UnitHandler();
  // const { getAddressByIdHandler } = AddressHan
  // dler();
const { handleAddUserUnit } = UserUnitHandler();
...
useEffect(() => {
  if (mode === "edit" && editData) {
    // Construct the full URL if photo exists
    const photoUrl = editData.photo
      ? `${process.env.REACT_APP_BASE_URL || "http://localhost:5000"}/${editData.photo.replace(/\\/g, "/")}`
      : "";
    ...
    setProfilePhoto(photoUrl); // ✅ show photo preview
    setSelectedRoleId(editData.roleId);
  }
}, [mode, editData]);
...
//   // fetch all addresses and map by addressId
//   getAllAddressesHandler().then((addresses) => {
//     const address = addresses.find(
//       (a) => a.addressId === editData.addressId
//     );
...
const handleSelect = (unitId) => {
  setSelectedUnits((prev) => {
    ...
    if (prev.includes(unitId)) {
      updated = prev.filter((id) => id !== unitId);
    } else {
      updated = [...prev, unitId];
    }
...
const submitProfileUser = async () => {
  try {
    ...
    form.append("unitId", firstUnitId);

    // 🔥 IMPORTANT — SEND ADDRESS AS JSON
    ...
    const response = await createSocietyResidentUserHandler(
      societyId,
      form
    );
    ...
    if (!userId) {
      toast.error("User created but ID missing");
      return;
    }

    // 🔥 Assign remaining units
    const otherUnits = selectedUnits.slice(1);

    for (const unitId of otherUnits) {
      await handleAddUserUnit({ userId, unitId });
    }

    toast.success("User created successfully with all units!");
...
//   // Step 1: Save user with the first unit
//   const firstUnitId = selectedUnits[0];
...
//   form.append("mobileNumber", formData.mobileNumber);
//   form.append("roleId", selectedRoleId);
//   form.append("unitId", firstUnitId); // main unit
//   form.append("address", JSON.stringify(formData.address));
...
//   try {
//     // Create user with first unit
//     const userResponse = await createSocietyResidentUserHandler(societyId, form);
...
//     // Step 2: Assign additional units to UserUnit table
//     const otherUnits = selectedUnits.slice(1); // all except the first
//     for (const unitId of otherUnits) {
//       try {
//         await handleAddUserUnit(unitId); // calls backend to insert into UserUnit table
//       } catch (err) {
//         console.error(`Failed to assign unit ${unitId}:`, err);
//         toast.error(`Failed to assign unit ${unitId}: ${err.message}`);
//       }
//     }

//     toast.success("All units assigned successfully!");
//   } catch (error) {
//     console.error("Error creating resident:", error);
//     toast.error("Failed to create user profile.");
//   }
...
<div className="max-w-sm">
  ...
  {formData.ismaemberofassociationcommite && (
    <Input
  ...
  <div className="mt-8">
    ...
    <div className="mt-4 text-sm text-gray-600">
      ...
      <div className="flex justify-center mt-5">
        ...
        <Button  className="max-w-sm" size="lg"
          type="button" onClick={submitProfileUser}>
