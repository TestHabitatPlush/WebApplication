...
//       try {
//         const purposeRes = await getTicketDropdown();
//         console.log("Dropdown fetched:", purposeRes); // ✅ debug
//         // ✅ FIX: only set the `data` array, not the whole object
//         setTicketPurpousList(purposeRes.data || []);
//       } catch (err) {
//         console.error("Error fetching ticket purpose", err.message);
//       }
//     };
...
const CreateTicketForm = () => {
  ...
  const societyId = useSelector((state) => state.society.selectedSocietyId); // ✅ wait for societyId
  const token = useSelector((state) => state.auth.token); // ✅ wait for token
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const purposeRes = await getTicketDropdown();
        console.log("Dropdown fetched:", purposeRes); 
        setTicketPurpousList(purposeRes); 
      } catch (err) {
        console.error("Error fetching ticket purpose", err.message);
      }
