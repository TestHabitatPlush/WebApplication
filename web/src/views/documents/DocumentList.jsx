...
const DocumentList = ({ refreshTrigger }) => {
  ...
  const [filterType, setFilterType] = useState('all'); // all, self, society

  const { getDocumentByUserHandler, getDocumentBySocietyHandler, deleteDocumentHandler } = DocumentHandler();
  const token = useSelector((state) => state.auth.token);
  const userId = 4;
  //useSelector((state) => state.auth.userId);
  const societyId =
  2;
  // useSelector((state) => state.auth.societyId);
  ...
  const fetchDocuments = async () => {
    ...
    try {
      ...
      if (filterType === 'self') {
        docs = await getDocumentByUserHandler();
      } else if (filterType === 'society') {
        const res = await getDocumentBySocietyHandler(societyId, userId, token);
        docs = res || [];
      } else if (filterType === 'all') {
        const [userDocs, societyDocs] = await Promise.all([
          getDocumentByUserHandler(),
          getDocumentBySocietyHandler(societyId, userId, token),
        ]);
        docs = [...(userDocs || []), ...(societyDocs || [])];
      }
